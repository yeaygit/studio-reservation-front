import { useDeferredValue, useEffect, useState, ChangeEvent, type FormEvent } from 'react'
import { ArrowUpRight, FileQuestion, Plus, Search, Sparkles } from 'lucide-react'
import customAxios from '../../utils/customAxios'
import { CRUD_PAGE_MODES as FAQ_MODES, type CrudPageMode } from '../../constants/pageModes'
...
import type { Faq, FaqFormState } from '../../types/content'
...
const createSummary = (value: string): string => {
...
const FaqManagementPage = () => {
  ...
  const [mode, setMode] = useState<CrudPageMode>(FAQ_MODES.LIST)
  ...
  const getFaqs = async (): Promise<Faq[]> => {
    try {
      const response = await customAxios.get<Faq[]>('/admin/faq')
      const normalizedFaqs = response.data.map((res: Faq) => normalizeFaq(res))
      setFaqs(normalizedFaqs)
      return normalizedFaqs
    } catch (error: unknown) {
      setSuccessMessage('')
      setErrorMessage(getApiErrorMessage(error, 'FAQ 목록을 불러오지 못했습니다.'))
      return []
    }
  }

  const getFaqById = async (faqId: number): Promise<Faq | null> => {
    try {
      const response = await customAxios.get<Faq>(`/admin/faq/${faqId}`)
      return response.data
    } catch (error: unknown) {
      setSuccessMessage('')
      setErrorMessage(getApiErrorMessage(error, 'FAQ 상세 정보를 불러오지 못했습니다.'))
      return null
    }
  }
  ...
  const handleSelectFaq = async (faq: Faq) => {
    ...
    if (!faq.id) {
      setErrorMessage('FAQ ID가 없어 상세 정보를 불러올 수 없습니다.')
      return
    }
  ...
  const handleOpenEdit = () => {
    ...
    setForm({
      ...
      sortOrder: selectedFaq.sortOrder ?? null,
    })
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload: Partial<Faq> = { question, answer }
    ...
    try {
      const response =
        mode === FAQ_MODES.CREATE
          ? await customAxios.post<Faq>('/admin/faq', payload)
          : await customAxios.patch<Faq>(`/admin/faq/${selectedFaq!.id}`, payload)
      ...
      setSuccessMessage('')
      setErrorMessage(getApiErrorMessage(error, 'FAQ 저장에 실패했습니다.'))
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSuccessMessage('')
      setErrorMessage(getApiErrorMessage(error, 'FAQ 삭제에 실패했습니다.'))
    }
  }
