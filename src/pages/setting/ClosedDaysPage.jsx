...
import { CalendarDays, Plus, RefreshCw, Repeat, Trash2 } from 'lucide-react'
import DatePicker from '../../component/common/DatePicker'
import SelectBox from '../../component/common/SelectBox'
import YearSelect from '../../component/common/YearSelect'
...
const createInitialForm = (year = CURRENT_YEAR, type = CLOSED_DAY_TYPES.SPECIFIC) => {
...
const extractErrorMessage = (error, fallbackMessage) =>
  error?.response?.data?.errorMessage || fallbackMessage
...
const formatRuleLabel = (closedDay) => {
...
const sortClosedDays = (closedDays) =>
...
const ClosedDaysPage = () => {
  ...
  const [form, setForm] = useState(() => createInitialForm(CURRENT_YEAR))
  ...
  const loadClosedDays = async (yearValue) => {
    setErrorMessage('')

    try {
      const response = await customAxios.get("/admin/settings/closed-days", {
        params: {
      ...
      setClosedDays([])
      setErrorMessage(extractErrorMessage(error, '휴무일 목록을 불러오지 못했습니다.'))
    } 
  }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setErrorMessage(extractErrorMessage(error, '휴무일을 등록하지 못했습니다.'))
    }
  }

  const handleDelete = async (closedDayId) => {
    ...
    try {
      ...
      setErrorMessage(extractErrorMessage(error, '휴무일을 삭제하지 못했습니다.'))
    }
  }
