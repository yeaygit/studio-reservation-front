...
const normalizeStudioSetting = (setting) => ({
  ...
  slotUnit: setting?.slotUnit ,
  reservationOpenDays: setting?.reservationOpenDays,
...
const areFormsEqual = (left, right) =>
  Object.keys(INITIAL_FORM).every((key) => left?.[key] === right?.[key])
...
const StudioSettingPage = () => {
  ...
  useEffect(() => {
    const loadStudioSetting = async () => {
      ...
      try {
        const response = await customAxios.get('/admin/settings/studio')
        const normalizedSetting = normalizeStudioSetting(response.data)

        setForm(normalizedSetting)
        setSavedForm(normalizedSetting)
      } catch (error) {
        setErrorMessage(error.response.data.errorMessage || '스튜디오 운영 설정을 불러오지 못했습니다.')
      }
    }
  ...
  const handleClosedDayChange = ({ target: { name, checked } }) => {
    setForm((previous) => ({
      ...previous,
      [name]: checked,
    }))
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await customAxios.patch('/admin/settings/studio', payload)
      const normalizedSetting = normalizeStudioSetting(response.data)
      ...
      setErrorMessage(error.response.data.errorMessage || '스튜디오 운영 설정을 저장하지 못했습니다.')
    }
  }
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <fieldset className={styles.settingSection}>
            ...
            <div className={styles.dayGrid}>
              {CLOSED_DAY_OPTIONS.map(({ key, label, shortLabel }) => (
                <label
                  key={key}
