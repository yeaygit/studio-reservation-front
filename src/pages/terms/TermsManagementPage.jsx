...
import { CRUD_PAGE_MODES as TERMS_MODES } from '../../constants/pageModes'
...
const TermsManagementPage = () => {
  ...
  const [mode, setMode] = useState(TERMS_MODES.LIST)
  ...
  const getTerms = async () => {
    try {
      const response = await customAxios.get('/admin/terms')
      ...
      setSuccessMessage('')
      setErrorMessage(
        error.response?.data?.errorMessage || '약관 목록을 불러오지 못했습니다.',
      )
      return []
    }
  }
  ...
  const getTermsById = async (termsId) => {
    try {
      const response = await customAxios.get(`/admin/terms/${termsId}`)
      return response.data
    } catch (error) {
      setSuccessMessage('')
      setErrorMessage(
        error.response?.data?.errorMessage || '약관 상세 정보를 불러오지 못했습니다.',
      )
      return null
    }
  }
  ...
  const handleSelectTerms = async (terms) => {
    clearMessages()
    ...
    try {
      const response =
        mode === TERMS_MODES.CREATE
          ? await customAxios.post('/admin/terms', payload)
          : await customAxios.patch(`/admin/terms/${selectedTerms.id}`, payload)
      ...
      setSuccessMessage('')
      setErrorMessage(
        error.response?.data?.errorMessage || '약관 저장에 실패했습니다.',
      )
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSuccessMessage('')
      setErrorMessage(
        error.response?.data?.errorMessage || '약관 삭제에 실패했습니다.',
      )
    }
  }
