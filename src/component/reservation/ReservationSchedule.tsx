...
import type {
  ...
  ReservationSettingResponse,
  ReservationSlot,
} from '../../types/reservation'

// 오늘부터 예약 오픈 일수만큼의 선택 가능 날짜 범위를 만든다.
const getReservationWindow = (reservationOpenDays: number): { startDate: string; endDate: string } => {
...
const getBookedRange = (reservation: BookedReservation): BookedRange | null => {
...
const hasOverlap = (start: number, end: number, bookedList: BookedReservation[]): boolean =>
...
const canReserve = (
  start: number,
  totalMinutes: number,
  setting: ReservationSettingResponse,
  bookedList: BookedReservation[],
): boolean => {
...
const generateSlots = (
  setting: ReservationSettingResponse | null,
  totalMinutes: number,
  bookedList: BookedReservation[],
): ReservationSlot[] => {
  if (
...
const ReservationSchedule = ({ form, setForm, typeList }: ReservationScheduleProps) => {
  const [reservationSetting, setReservationSetting] = useState<ReservationSettingResponse | null>(null)
  const [bookedListByDate, setBookedListByDate] = useState<BookedReservation[]>([])
  ...
  const loadReservationSetting = async () => {
    ...
    try {
      const response = await customAxios.get<ReservationSettingResponse>('/v1/reservations/setting')
  ...
  const changeDateSelect = (dateString: string): void => {
    setForm((previous) => ({
      ...previous,
      date: dateString,
  ...
  const changeTimeSelect = (time: string): void => {
  ...
  const isSelectedRange = (time: string): boolean => {
  ...
  const loadBookedListByDate = async () => {
    ...
    try {
      const response = await customAxios.get<BookedReservation[]>('/v1/reservations/booked-times', {
        params: {
          date: form.date,
        },
      })
